...
export const getAppliance = async (id: string): Promise<ApplianceRecord> => {
  const { data } = await axios.get<ApplianceRecord>(`${import.meta.env.VITE_API_URL}/appliance/${id}`);
  return data;
};
...
export const updateAppliance = async (appliance: BaseAppliance, id: string): Promise<BaseAppliance> => {
  const { data } = await axios.put<BaseAppliance>(`${import.meta.env.VITE_API_URL}/appliance/${id}`, appliance);
  return data;
};
